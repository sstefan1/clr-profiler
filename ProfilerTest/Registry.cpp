...
STDAPI DllRegisterServer() {
  ...
  HRESULT rc = RegisterServer(hProfiler,      // DLL module handle
                              CLSID_Profiler, // Class ID
                              FName,          // Friendly Name
                              VerInd,         // Programmatic
                              ProgId);        //   IDs
...
STDAPI DllUnregisterServer() {
  HRESULT rc = UnregisterServer(CLSID_Profiler, VerInd, ProgId);
...
STDAPI DllInstall(char *s) { return S_OK; };
                       ...
                       const WCHAR *szProgID)       //   IDs
{
  ...
  WCHAR szModule[512];
  DWORD dwResult =
      GetModuleFileName(hModule, szModule, sizeof(szModule) / sizeof(WCHAR));
  assert(dwResult != 0);
